...
# Public Subnet (for Instance A - Vote/Result + Bastion)
...
# Private Subnet for Redis/Worker (Instance B)
resource "aws_subnet" "private_redis" {
  vpc_id            = aws_vpc.main.id
  cidr_block        = var.private_subnet_redis_cidr
  availability_zone = var.availability_zone

  tags = {
    Name = "${var.project_name}-private-subnet-redis"
  }
}

# Private Subnet for PostgreSQL (Instance C)
